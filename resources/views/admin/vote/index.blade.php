            ...
            <div class="col-md-12">
                ...
                <div class="panel panel-inverse" data-sortable-id="table-basic-5">
                    ...
                    <div class="panel-body">
                        @if(auth('admin')->user()->can('vote.add'))
                        <a href="{{ url('admin/vote/create') }}">
                        ...
                        <table class="table table-bordered table-hover" id="datatable">
                            ...
                            <tr>
                                ...
                                <td>@if($val->status == 1)
                                        <button class="btn btn-success btn-xs">发布中</button>
                                    @elseif($val->status == 2)
                                     <button class="btn btn-success btn-xs">投票中</button>
                                     @else
                                     <button class="btn btn-error btn-xs">已结束</button>
                                    @endif
                                </td>
                                ...
                                <td>
                                    ...
                                    @if($val->status == 1)
                                        <a href="{{ url('admin/vote/'.$val->id) }}">
                                            <button type="button" class="btn btn-success btn-xs"> 结束投票
                                            </button>
                                        </a>

                                        <a href="{{ url('admin/vote/'.$val->id.'/edit') }}"><button type="button" class="btn btn-success btn-xs"><i class="fa fa-pencil"> 编辑</i></button></a>

                                        <a href="{{ url('admin/person?vote_id='.$val->id) }}"><button type="button" class="btn btn-success btn-xs"><i class="fa fa-pencil"> 候选人管理</i></button></a>
                                    @endif

                                    <a href="javascript:;" data-id="{{$val->id}}" class="btn btn-danger btn-xs destroy"><i class="fa fa-trash"> 删除</i>
                                        <form action="{{ url('admin/vote/'.$val->id) }}" method="POST" name="delete_item_{{$val->id}}" style="display:none">
                                        {{ csrf_field() }}
...
@section('admin-js')
    ...
    <script src="{{ asset('asset_admin/js/vote.list.js')}}"></script>
    <script>
        $(function(){

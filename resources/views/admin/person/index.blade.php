            ...
            <div class="col-md-12">
                ...
                <div class="panel panel-inverse" data-sortable-id="table-basic-5">
                    ...
                    <div class="panel-body">
                        @if(auth('admin')->user()->can('person.add'))
                        <a href="{{ url('admin/person/create') }}">
                        ...
                        <table class="table table-bordered table-hover" id="datatable">
                            ...
                            <tr>
                                <td>{{$val->id}}</td>
                                <td>
                                    <img src="{{url('uploads/'.$val->avatar)}}" style="width: 200px">
                                </td>
                                <td>{{$val->name}}</td>
                                <td>
                                    <a href="{{ url('admin/person/'.$val->id.'/edit') }}"><button type="button" class="btn btn-success btn-xs"><i class="fa fa-pencil"> 编辑</i></button></a>

                                    <a href="javascript:;" data-id="{{$val->id}}" class="btn btn-danger btn-xs destroy"><i class="fa fa-trash"> 删除</i>
                                    <form action="{{ url('admin/person/'.$val->id) }}" method="POST" name="delete_item_{{$val->id}}" style="display:none">
                                        {{ csrf_field() }}
